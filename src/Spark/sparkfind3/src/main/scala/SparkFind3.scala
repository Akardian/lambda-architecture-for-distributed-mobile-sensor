...
object SparkFind3 {
    ...
    def main(args: Array[String]) {
        ...
        val hdfsDataFrame = avroDataFrame
            .withColumn("timestamp-hdfs", date_format(date_trunc("hour", $"timestamp-Kafka-in"), "yyyy-MM-dd HH-mm"))
        ...
        */
        ...
        val query = hdfsDataFrame
            .selectExpr("CAST(timestamp-Kafka-in AS STRING) as timestamp-Kafka-in", "to_json(struct(*)) AS value")
            .writeStream
            .format("kafka")
            .outputMode("update")
            .option("kafka.bootstrap.servers", BOOTSTRAP_SERVERS)
